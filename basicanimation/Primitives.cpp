...
void Joint::draw() {
   ...
   //
	...
	{
		ofPushMatrix();
		glm::vec3 v = parent->getPosition() - getPosition();
		glm::vec3 v2 = glm::normalize(v);
		glm::mat4 rot = rotateToVector(glm::vec3(0, 1, 0), v2);
		glm::mat4 trans = glm::translate(getPosition());
		float len = glm::length(v);
		glm::mat4 offset = glm::translate(glm::vec3(0.0f, len/2.0f, 0.0f));
		ofMultMatrix(trans * rot * offset);
		bone.setHeight(len - 2.0f*radius);
		bone.draw();
		ofPopMatrix();
...
//intersect method of joint ,same as sphere
