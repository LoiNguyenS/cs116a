...
//
//  
// Modified some of Prosessor Kevin Smiths code and used
//Loi Nguyen
//added joint class that extends sceneobject
#pragma once
...
public:
	Cone(ofColor color = ofColor::red) {
		diffuseColor = color;
	}
...
public: 
	Joint(glm::vec3 p, float r, string s, ofColor diffuse = ofColor::lightGray) { position = p; radius = r; diffuseColor = diffuse; SceneObject::name = s; }
	Joint() {}
	bool intersect(const Ray &ray, glm::vec3 &point, glm::vec3 &normal);
	void draw();
	float radius = 1.0;
